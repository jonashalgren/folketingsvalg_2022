...
import type { C_Scene_Progress } from "@models";
...
export function setMapper(item: Props): Props {
  ...
  const mapperPositionExit = interpolate(
    [0, 1],
    [cameraSettings.positionMain.outputRange[cameraSettings.positionMain.outputRange.length - 1], cameraSettings.positionExit]
  );
  ...
  const mapperTargetExit = interpolate(
    [0, 1],
    [cameraSettings.targetMain.outputRange[cameraSettings.targetMain.outputRange.length - 1], cameraSettings.targetExit]
  );
  const mapperTargetEntry = interpolate([0, 1], [cameraSettings.targetEntry, cameraSettings.targetMain.outputRange[0]]);

  return {
    ...item,
    mapper: function (progress: C_Scene_Progress) {
      return {
