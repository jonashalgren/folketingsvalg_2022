...
export function getCameraTargetMapper({ data }: Props) {
  const { inputRange, outputRange } = data.camera.target;
  const { entryTarget, exitTarget } = data.camera;
  ...
  return {
    ...
    cameraEntryTargetMapper: function ({ progress, controls }: { progress: number; controls: OrbitControls }) {
