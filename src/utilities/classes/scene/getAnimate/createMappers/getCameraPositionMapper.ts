...
export function getCameraPositionMapper({ data }: Props) {
  ...
  const { outputRange, inputRange } = data.camera.position;
  const { entryPosition, exitPosition } = data.camera;
  ...
  return {
    cameraMainPositionMapper: function ({ progress, camera }: { progress: number; camera: PerspectiveCamera }) {
      if (localProgress !== progress) {
        localProgress = progress;
        camera.position.set(...progressMapper(progress));
      }
    },
    cameraExitPositionMapper: function ({ progress, camera }: { progress: number; camera: PerspectiveCamera }) {
      if (localExit !== progress) {
        localExit = progress;
        camera.position.set(...exitMapper(progress));
      }
    },
    cameraEntryPositionMapper: function ({ progress, camera }: { progress: number; camera: PerspectiveCamera }) {
