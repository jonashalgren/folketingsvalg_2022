...
import type { C_C_E_Mesh_Figure } from "@models";
...
export function getFigureMeshes({ data }: Props) {
  return data.paths.flatMap((path) => {
    return SVGLoader.createShapes(path).map((shape) => {
      ...
      const material = getLambertMaterial({ color: new Color(mesh_static_settings.color_dark) });
      ...
      return mesh as C_C_E_Mesh_Figure;
    });
  });
}
